...
class AlphaVantageAPI:
    ...
    def get_quote(self, symbol: str) -> Optional[Dict]:
        """Get real-time quote data with improved error handling"""
        for attempt in range(3):  # Try up to 3 times
            try:
                ...
                print(f"🔍 Fetching quote for {symbol} (attempt {attempt + 1})...")
                ...
                if "Note" in data:
                    ...
                    if "rate limit" in str(data['Note']).lower():
                ...
                if "Global Quote" not in data:
                    print(f"❌ No Global Quote in response: {data}")
                    return None
                
                print(f"✅ Successfully fetched quote for {symbol}")
                return data
                
            except requests.exceptions.ConnectionError as e:
                print(f"🌐 Connection error for {symbol} (attempt {attempt + 1}): {e}")
                ...
                print(f"⏰ Timeout fetching quote for {symbol} (attempt {attempt + 1}): {e}")
                if attempt < 2:
                    time.sleep(5)
                continue
                
            except requests.exceptions.RequestException as e:
                print(f"🌐 Request error for {symbol} (attempt {attempt + 1}): {e}")
                if attempt < 2:
                    time.sleep(5)
                continue
...
async def test_alpha_vantage_connection():
    ...
    try:
        ...
        if test_data and 'Global Quote' in test_data:
            ...
            print(f"⚠️ Alpha Vantage response received but unexpected format: {test_data}")
            return False
        else:
            print("❌ Alpha Vantage connection failed - no data returned")
            return False
