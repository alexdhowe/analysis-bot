            ...
            if hasattr(yahoo_feed, 'entries') and yahoo_feed.entries:
                for entry in yahoo_feed.entries[:5]:
                    try:
                        ...
                        print(f"⚠️ Error parsing news entry: {e}")
                        continue
...
async def on_ready():
    ...
    api_working = await test_polygon_connection()
    if api_working:
        print("✅ Polygon.io API is working correctly")
    else:
        print("❌ Polygon.io API test failed - check your API key")
...
async def stock_command(ctx, symbol: str = None):
    ...
    loading_msg = await ctx.send(f"🔍 Analyzing **{symbol}**... Please wait...")
    ...
    try:
        ...
        if analysis_data:
            ...
            await loading_msg.edit(
                content=f"❌ Could not analyze **{symbol}**. Please check:\n"
        ...
        await loading_msg.edit(
            content=f"❌ An error occurred while analyzing {symbol}. Please try again later."
        )
...
if __name__ == "__main__":
    ...
    if not TOKEN:
        ...
        print("Please set your Discord bot token in the .env file or Render environment variables")
        sys.exit(1)
    
    if not POLYGON_API_KEY:
        print("⚠️ WARNING: POLYGON_API_KEY not found - stock analysis will not work!")
