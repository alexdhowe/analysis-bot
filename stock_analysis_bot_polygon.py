...
def analyze_options_strategy(symbol: str, analysis_data: dict) -> Dict:
    ...
    """Create options suggestions embed"""
    embed = discord.Embed(
        title=f"💎 Options Plays: {symbol}",
        description="High-probability options strategies under $3.00",
        color=0x9932cc,
        timestamp=datetime.now()
    )
    
    try:
        print(f"🔍 Analyzing options for {symbol}...", flush=True)
        
        if not polygon_api:
            embed.add_field(
                name="❌ Options Analysis Unavailable",
                value="Polygon.io API not configured",
                inline=False
            )
            return embed
        ...
        options_data = await asyncio.get_event_loop().run_in_executor(
            None, polygon_api.get_options_chain, symbol
        )
        
        if not options_data:
            embed.add_field(
                name="❌ No Options Data",
                value="Could not retrieve options chain for this symbol",
                inline=False
            )
            return embed
        
        if options_data.get('error') == 'premium_required':
            embed.add_field(
                name="🔒 Premium Feature",
                value="Options data requires a paid Polygon.io subscription.\n\n**Free Alternative Suggestions:**\n• Check your broker's options chain\n• Use Yahoo Finance or other free tools\n• Consider upgrading to Polygon.io paid tier",
                inline=False
            )
            
            # Add basic directional suggestions based on technical analysis
            current_sentiment = analysis_data.get('sentiment', '⚪ NEUTRAL')
            score = analysis_data.get('score', 0)
            current_price = analysis_data.get('current_price', 0)
            
            suggestions_text = ""
            
            if "BULLISH" in current_sentiment and score > 1:
                suggestions_text += "📈 **Bullish Bias Detected**\n"
                suggestions_text += f"• Consider **CALL options** with strikes near ${current_price:.0f}\n"
                suggestions_text += "• Look for 2-4 weeks expiration\n"
                suggestions_text += "• Target strikes 2-5% OTM for higher probability\n\n"
            elif "BEARISH" in current_sentiment and score < -1:
                suggestions_text += "📉 **Bearish Bias Detected**\n"
                suggestions_text += f"• Consider **PUT options** with strikes near ${current_price:.0f}\n"
                suggestions_text += "• Look for 2-4 weeks expiration\n"
                suggestions_text += "• Target strikes 2-5% OTM for higher probability\n\n"
            else:
                suggestions_text += "⚪ **Neutral/Mixed Signals**\n"
                suggestions_text += "• Consider **range-bound strategies**\n"
                suggestions_text += "• Iron condors or strangles might be suitable\n"
                suggestions_text += "• Wait for clearer directional signals\n\n"
            
            suggestions_text += f"**Key Levels:**\n"
            suggestions_text += f"• Support: ${analysis_data.get('support', current_price * 0.95):.2f}\n"
            suggestions_text += f"• Resistance: ${analysis_data.get('resistance', current_price * 1.05):.2f}\n"
            suggestions_text += f"• Current RSI: {analysis_data.get('rsi', 50):.1f}"
            
            embed.add_field(
                name="💡 Technical-Based Suggestions",
            ...
            embed.add_field(
                ...
                value="Options trading involves significant risk. These are educational suggestions only, not financial advice. Always do your own research and risk management.",
                inline=False
            )
            
            return embed
        
        # If we had real options data, we would analyze it here
        options_contracts = options_data.get('results', [])
        expiration_date = options_data.get('expiration_date', 'Unknown')
        
        if not options_contracts:
            embed.add_field(
                name="❌ No Contracts Found",
                value="No options contracts found meeting the criteria",
                inline=False
            )
            return embed
        
        # Analyze options (this would be the full implementation)
        embed.add_field(
            name="📊 Options Analysis",
            value=f"Found {len(options_contracts)} contracts expiring {expiration_date}",
            inline=False
        )
        ...
        embed.add_field(
            name="❌ Analysis Error",
            value=f"Could not analyze options: {str(e)}",
            inline=False
        )
