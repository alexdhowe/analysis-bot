...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.exit(1)current_quote_task, prev_close_task, historical_task
        )
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value=(
            #!/usr/bin/env python3
"""
Enhanced Stock Analysis Bot with Polygon.io API - COMPLETE VERSION WITH OPTIONS
24/7 stock analysis with professional-grade API and options strategies
"""
...
class PolygonAPI:
    """Polygon.io API wrapper - COMPLETE VERSION WITH OPTIONS"""
    ...
    def get_options_chain(self, symbol: str, expiration_date: str = None) -> Optional[Dict]:
        ...
        try:
            # If no expiration provided, get the next available expiration
            if not expiration_date:
                # Get available expirations first
                exp_url = f"{self.base_url}/v3/reference/options/contracts"
                exp_params = {
                ...
                exp_response = self.session.get(exp_url, params=exp_params, timeout=30)
                if exp_response.status_code == 200:
                    exp_data = exp_response.json()
                    if 'results' in exp_data and exp_data['results']:
                        # Find expiration more than 7 days out
                        one_week_out = (datetime.now() + timedelta(days=7)).strftime('%Y-%m-%d')
                        for contract in exp_data['results']:
                            if contract.get('expiration_date', '') > one_week_out:
                                expiration_date = contract['expiration_date']
                                break
                
                if not expiration_date:
                    print(f"❌ No suitable expiration found for {symbol}", flush=True)
                    return None
            
            # Get options quotes for the expiration
            ...
            print(f"🔍 Fetching options chain for {symbol} expiring {expiration_date}...", flush=True)
            ...
            if response.status_code == 403:
                print(f"❌ Options data not available on free tier for {symbol}", flush=True)
                return {'error': 'premium_required', 'message': 'Options data requires paid subscription'}
            ...
            if data.get('status') in ['OK', 'DELAYED'] and 'results' in data:
                ...
                print(f"✅ Successfully fetched {len(results)} options contracts for {symbol}", flush=True)
                return {
                    'results': results,
                ...
                print(f"❌ No options data for {symbol}: {data}", flush=True)
                return None
                
        except Exception as e:
            print(f"❌ Error fetching options chain for {symbol}: {e}", flush=True)
            return None
...
class StockAnalysisView(discord.ui.View):
    """Interactive view for stock analysis - FIXED VERSION WITH OPTIONS"""
    ...
    @discord.ui.button(label="💎 Options Plays", style=discord.ButtonStyle.danger, emoji="⚡")
    async def options_suggestions(self, interaction: discord.Interaction, button: discord.ui.Button):
        print(f"💎 Options suggestions button clicked by {interaction.user} for {self.symbol}", flush=True)
        await interaction.response.defer(ephemeral=True)
        
        try:
            embed = await create_options_suggestions_embed(self.symbol, self.analysis_data)
            await interaction.followup.send(embed=embed, ephemeral=True)
            print(f"✅ Sent options suggestions for {self.symbol}", flush=True)
        except Exception as e:
            print(f"❌ Error in options suggestions: {e}", flush=True)
            await interaction.followup.send(f"❌ Error fetching options: {str(e)}", ephemeral=True)
...
def analyze_options_strategy(symbol: str, analysis_data: dict) -> Dict:
    """Analyze best options strategy based on technical analysis"""
    
    current_price = analysis_data.get('current_price', 0)
    sentiment = analysis_data.get('sentiment', '⚪ NEUTRAL')
    score = analysis_data.get('score', 0)
    rsi = analysis_data.get('rsi', 50)
    support = analysis_data.get('support', current_price * 0.95)
    resistance = analysis_data.get('resistance', current_price * 1.05)
    
    # Calculate target strikes based on technical levels
    otm_call_strikes = []
    otm_put_strikes = []
    
    # Generate potential strikes (simplified calculation)
    for i in range(1, 6):  # 5 strikes above and below
        call_strike = current_price + (current_price * 0.025 * i)  # 2.5% increments
        put_strike = current_price - (current_price * 0.025 * i)
        
        if call_strike < resistance * 1.1:  # Don't go too far OTM
            otm_call_strikes.append(call_strike)
        if put_strike > support * 0.9:
            otm_put_strikes.append(put_strike)
    
    strategies = []
    
    # Bullish strategies
    if "BULLISH" in sentiment and score > 1:
        confidence = "High" if score > 3 else "Medium"
        
        for strike in otm_call_strikes[:3]:  # Top 3 strikes
            prob_profit = max(60 - (abs(strike - current_price) / current_price * 100 * 10), 30)
            ...
            strategies.append({
                ...
                'target': resistance,
                'reasoning': f"Technical score {score}/10, RSI {rsi:.1f}",
                'max_risk': 'Premium paid (~$1.50-$3.00)',
                'breakeven': f"${strike + 2:.2f} (strike + premium)"
            })
    
    # Bearish strategies  
    if "BEARISH" in sentiment and score < -1:
        confidence = "High" if score < -3 else "Medium"
        
        for strike in otm_put_strikes[:3]:
            prob_profit = max(60 - (abs(current_price - strike) / current_price * 100 * 10), 30)
    ...
    if abs(score) < 1 or rsi > 70 or rsi < 30:
        # Suggest range-bound strategies
        strategies.append({
            'type': 'Iron Condor',
            'direction': 'Neutral/Range',
            'strike': f"${support:.2f} - ${resistance:.2f}",
            'confidence': 'Medium',
            'prob_profit': 65,
            'target': 'Range bound movement',
            'reasoning': f"Neutral signals, RSI {rsi:.1f}",
            'max_risk': 'Net debit (~$2.00-$3.00)',
            'breakeven': 'Within the range'
        })
    
    # Sort by probability of profit
    strategies.sort(key=lambda x: x['prob_profit'], reverse=True)
...
async def create_options_suggestions_embed(symbol: str, analysis_data: dict) -> discord.Embed:
    """Create options suggestions embed with technical analysis"""
    embed = discord.Embed(
        title=f"💎 Options Plays: {symbol}",
        description="AI-powered options strategies based on technical analysis",
        color=0x9932cc,
        timestamp=datetime.now()
    )
    
    try:
        print(f"🔍 Analyzing options strategies for {symbol}...", flush=True)
        
        # Perform options strategy analysis
        options_analysis = analyze_options_strategy(symbol, analysis_data)
        
        current_price = options_analysis['current_price']
        strategies = options_analysis['strategies']
        overall_bias = options_analysis['overall_bias']
        
        # Market Overview
        embed.add_field(
            name="📊 Market Overview",
            value=f"**Current Price:** ${current_price:.2f}\n**Bias:** {overall_bias}\n**Support:** ${options_analysis['support']:.2f}\n**Resistance:** ${options_analysis['resistance']:.2f}",
            inline=False
        )
        
        if not strategies:
            embed.add_field(
                name="⚪ No Clear Opportunities",
                value="Current technical analysis doesn't show strong directional bias. Consider waiting for clearer signals or neutral strategies.",
                inline=False
            )
        else:
            # Show top 3 strategies
            for i, strategy in enumerate(strategies[:3], 1):
                direction_emoji = "📈" if strategy['direction'] == 'Bullish' else "📉" if strategy['direction'] == 'Bearish' else "⚪"
                confidence_emoji = "🔥" if strategy['confidence'] == 'High' else "💪" if strategy['confidence'] == 'Medium' else "🤷"
                
                strategy_text = f"{direction_emoji} **{strategy['type']}**\n"
                
                if strategy['type'] != 'Iron Condor':
                    strategy_text += f"**Strike:** ${strategy['strike']:.2f}\n"
                else:
                    strategy_text += f"**Range:** {strategy['strike']}\n"
                
                strategy_text += f"**Success Probability:** {strategy['prob_profit']:.0f}%\n"
                strategy_text += f"**Max Risk:** {strategy['max_risk']}\n"
                strategy_text += f"**Breakeven:** {strategy['breakeven']}\n"
                strategy_text += f"**Reasoning:** {strategy['reasoning']}"
                
                embed.add_field(
                    name=f"{confidence_emoji} Strategy #{i} - {strategy['confidence']} Confidence",
                    value=strategy_text,
                    inline=False
                )
        ...
        embed.add_field(
            name="📅 Expiration Guidance",
            value="**Recommended:** 2-4 weeks out\n**Minimum:** 1 week (avoid weekly expiries)\n**Sweet Spot:** 14-21 DTE for theta optimization",
            inline=False
        )
        ...
        embed.add_field(
            ...
            value="• Never risk more than 2% of portfolio per trade\n• Set profit targets at 50% of premium\n• Cut losses at -50% of premium\n• Avoid earnings announcements",
            inline=False
        )
        ...
        if polygon_api:
            embed.add_field(
                ...
                value="Based on technical analysis signals, support/resistance levels, and momentum indicators. Real-time options data requires premium subscription.",
                inline=False
            )
        
        embed.set_footer(text="💎 Options strategies • High risk • Not financial advice • Do your own research")
        
    except Exception as e:
        print(f"❌ Error in options analysis: {e}", flush=True)
        embed.add_field(
            name="❌ Analysis Error",
            value=f"Could not analyze options strategies: {str(e)}",
            inline=False
        )
