#!/usr/bin/env python3
"""
Enhanced Stock Analysis Bot with Polygon.io API - FIXED VERSION
    ...
    def get_current_quote(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if data.get('status') in ['OK', 'DELAYED'] and 'ticker' in data:
                ...
                print(f"❌ No current quote data for {symbol}", flush=True)
                return None
    ...
    def get_previous_close(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if data.get('status') in ['OK', 'DELAYED'] and 'results' in data and len(data['results']) > 0:
                ...
                print(f"❌ No data in response for {symbol}", flush=True)
                return None
                
        except Exception as e:
            print(f"❌ Error fetching previous close for {symbol}: {e}", flush=True)
            return None
    ...
    def get_historical_data(self, symbol: str, days: int = 90) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if data.get('status') in ['OK', 'DELAYED'] and 'results' in data:
                ...
                print(f"❌ No historical data for {symbol}", flush=True)
                return None
...
def parse_polygon_data(symbol: str, prev_close_data: Dict, historical_data: List[Dict], current_quote_data: Optional[Dict] = None) -> Optional[Dict]:
    ...
    try:
        ...
        if current_quote_data and 'day' in current_quote_data:
            try:
                day_data = current_quote_data['day']
                if 'c' in day_data:
                    current_price = float(day_data['c'])
                    current_price_label = "Current Price"
                    prev_close = float(day_data['o'])
                ...
                print(f"⚠️ Could not parse real-time price: {e}", flush=True)
        
        # If no real-time data, use previous close change
        if current_change == 0:
...
async def perform_stock_analysis(symbol: str) -> Optional[Dict]:
    ...
    try:
        ...
        current_quote_data, prev_close_data, historical_data = await asyncio.gather(
            asyncio.get_event_loop().run_in_executor(None, polygon_api.get_current_quote, symbol),
            asyncio.get_event_loop().run_in_executor(None, polygon_api.get_previous_close, symbol),
            asyncio.get_event_loop().run_in_executor(None, polygon_api.get_historical_data, symbol, 90)
        )
