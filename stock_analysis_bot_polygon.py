...
async def on_message(message):
    """Process all messages for debugging - FIXED VERSION"""
    # Prevent duplicate processing
    if message.author == bot.user:
        return
    
    # Only process if this is not a duplicate
...
async def stock_command(ctx, symbol: str = None):
    """Main stock analysis command - FIXED VERSION"""
    ...
    user_key = f"{ctx.author.id}_{symbol.upper()}"
    if user_key in analysis_cache:
        cache_time = analysis_cache[user_key].get('timestamp', datetime.now())
        time_diff = (datetime.now() - cache_time).total_seconds()
        if time_diff < 10:  # Prevent spam within 10 seconds
    ...
    try:
        ...
        if analysis_data:
            ...
            analysis_cache[user_key] = analysis_data
...
async def on_error(event, *args, **kwargs):
    ...
    print(f"❌ Bot error in {event}: {args}", flush=True)
