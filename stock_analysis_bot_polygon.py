...
print(f"🔑 DISCORD_TOKEN present: {'Yes' if os.getenv('DISCORD_TOKEN') else 'No'}")
print(f"📈 POLYGON_API_KEY present: {'Yes' if os.getenv('POLYGON_API_KEY') else 'No'}")
print(f"🌐 PORT: {os.getenv('PORT', '10000')}")

# Bot setup - DISABLE DEFAULT HELP COMMAND
intents = discord.Intents.default()
intents.message_content = True
print("🎯 Bot intents configured")
...
if __name__ == "__main__":
    ...
    health_thread.start()
    print("✅ Health check server thread started")
    ...
    if not TOKEN:
        ...
        print("Environment variables found:", list(os.environ.keys()))
        sys.exit(1)
    
    if not POLYGON_API_KEY:
        print("⚠️ WARNING: POLYGON_API_KEY not found - stock analysis will be limited!")
    
    print("🔑 Discord token found, attempting to login...")
